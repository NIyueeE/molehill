...
- 红色虚线是 **SLO**:交互流的 p99 应保持在 50 ms 以内。绿色带是健康的对照
  阶段,灰色带是被刻意劣化的阶段;最后一段重复 clean 条件,用来看恢复。
- 响应时间轴是**对数**的——劣化路径会贵出三个数量级,线性轴上健康阶段会被
  压成一条线。实线阶梯是每个阶段的中位数,虚线是它的 p99;底边的红条表示
  **wedge**:该流静默超过五秒。
- 第二张图把同一轮数据画成"每个条件一个面板"(圆点 = 中位数,横杠 = p99,
  竖线 = 最差一秒):当你只关心某一个条件而不是整条日程时,看这张。

以上是本主机上的 v0.9.0 数字,只有同模型、同主机的运行之间才可直接比较。
方法、阶段表、测试类型以及如何复现:[基准测试方法](docs/benchmarks.zh.md)。
