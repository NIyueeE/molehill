...
同一个二进制,**只变一个变量**——多路复用开关;`mux` 是对照组。覆盖
loopback 与全部弱网档。
...
- 多路复用用单流吞吐换取连接效率:同一个二进制 `mux = false` 时,loopback
  为 20.1 / 28.2 Gbit/s。
- 纯延迟下差距收窄(rtt100 单流几乎持平)——一旦 RTT 占主导,按连接建立
  的代价被摊薄。
- **丢包下并发流的差距反转**:单隧道共享同一个丢包/重传域(所有流一起
  停滞),`mux = false` 则每流独立重传——1% 丢包时 8 流吞吐 mux-off
  18.5 vs mux 4.7 Gbit/s。单流丢包行为几乎一致;多路复用的代价恰好
  在"所有流共命运"的地方显现。
