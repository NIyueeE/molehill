...
对端工具为最新 GitHub release 构建(frp 0.71.0、rathole 0.5.0 上游、nps
0.26.10)。网络档(netem 塑造整个 `lo`,每一跳都被延迟/丢包)与指标
...
**对端集合 2026-09-21 更新:bore 换成 nps 0.26.10**(ehang-io,部署广泛的
Go 多路复用器——比极简 TCP 转发器更有代表性的第三个对端)。上表与上图是
v0.8.1 发布测量,仍描述该次发布;下一次全矩阵跑会按新集合重新生成两者。
新集合在单主机上的回环复测(molehill 全严格度,对端 1 rep):

| 工具 | 1 流 | 8 流 | echo RTT p50 | 内存 | churn conn/s |
|---|---|---|---|---|---|
...
nps 是慢的批量转发者(每个字节两次进程跳跃且缓冲区小:~0.14 Gbit/s),
但不是慢的*连接建立者*——churn 速率与 echo RTT 都与大部队同档,也支持
UDP。它的队头探针测到 0,因为全双工批量 echo 会在其桥接里
stall(单向批量可跑 27 Gbit/s),图表将其渲染为缺失槽而非零值。

**注意(2026-09-21):** 以上两张表都跑在修复“死接收器泄漏”之前的引擎上
(该泄漏让服务大量短连接的客户端每轮 poll 扫上万个已完成流的接收器,
详见 `HANDOFF.md` 的 “What landed”)。修复后单隧道 8 流单元相对
修复前为 **+23.5%**,mux 默认臂回环 1/8 流为 +6.6%/+5.2%,churn 恢复至
main 水平;32 KiB 帧分割的旧否决(“Phase 3”)正是那次被污染测量的产物,
现已重新采用。v0.8.1 发布表格作为该次发布的记录保留,新数字在下一次
全矩阵发布跑中随新图表一起落位。
...
18.0);`mux1` 按设计没有该点(64 条流超过其单隧道 45 条数据流的可用
上限——即流数帽减去 bench 自持的池化通道与客户端控制流)。混合负载
...
- **指标**:TCP 吞吐(1/8/64 流——64 是 yamux 上限 `count × 64` 并发
...
- **已知限制**:回环不是真实网络(丢包/时延理想化,无真实拥塞);共享
  ...
  **UDP 的表征方式。** UDP 探针走一条限速短突发阶梯(500 pps 到配置的
  ...
  它不可复现(是波动,不是路径性质)。
  yamux 可用上限(流数帽 `count × 64` 减去 bench 自持的池化通道与
  客户端控制流)低于 64 流规模点的 arm 按设计跳过该探针(原因在
  `partial_metrics`)。图只画真正参与比较的行与格子(对端没跑的格子、
