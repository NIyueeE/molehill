...
![逐阶段、逐工具的交互流 RTT](assets/soak-v0.9.1-stages.png)
...
**Bulk 吞吐逐阶段**(Gbit/s):molehill clean 12.7 → rtt100 2.4 → rate100
0.03 → **回归 clean 16.2**;frp 6.0 → 2.1 → 5.9;rathole 12.4 → 2.5 → 12.4;
nps 全程 0.1。

**这些形状说明什么。** 每个工具在劣化档上都退化、在回归 clean 档上都恢复
——最后一段测的就是这个恢复;一个保持 wedge 的工具就是一个发现。交互流的 p99 才是新
访客真正感受到的东西:饱和状态下它是区分工具的那个数,也正是吞吐轴的盲区
——molehill 与 rathole 在 clean 档上 bulk 相同(12.7 对 12.4 Gbit/s),
而一次全新交互连接的代价是 9.3 ms 对 99 ms;在 1% 丢包档两者 bulk 都约 4.9
Gbit/s,交互流则是 1305 ms 对 1324 ms。对端由同一份工作负载驱动,画在同一批
面板里;漂移轴(全程的打开 fd、RSS 与 CPU 斜率)见
`soak-v0.9.1-drift.png`,UDP 会话的 RTT/丢包见 `soak-v0.9.1-udp.png`(画的是
滑动丢包**率**,不是丢包事件的计数)。

以上是本主机上的 v0.9.1 数字,只有同模型、同主机的运行之间才可直接比较。
clean 档的 bulk 低于 v0.9.0 那一轮在同一硬件上的 18.9/20.6 Gbit/s:把两个
**二进制**放进同一轮交错 A/B(`screen` 模式)后,它拒绝在两个方向上给出任何
claim——同一轮里 v0.9.0 的二进制自己也只有 7.6-24.5 Gbit/s 的波动范围,说明
移动的是主机当天的上限,而不是代码。可比性边界见
