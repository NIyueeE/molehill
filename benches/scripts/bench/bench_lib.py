...
def dump_results(data: dict, path: Path) -> None:
    ...
    The parent directory is recreated first: a run whose output path
    lives on a wiped filesystem (the /tmp cleanup reaped
    `/tmp/results-*.json` out from under a live run once) would
    otherwise die with a traceback on its next checkpoint, losing every
    arm measured so far — the checkpoint is the run's only durable
    state."""
