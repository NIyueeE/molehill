...
def main() -> None:
    ...
    # The pair is sorted by label, so A is NOT necessarily the left --ab
    # entry. When the run recorded the paths (bench.py meta.ab_bin_paths),
    # print them: a verdict read with the sides swapped inverts its
    # meaning (that mistake reverted a winning change once).
    paths = (data.get("meta") or {}).get("ab_bin_paths") or {}
    if paths:
        print("binaries (verdict reads A -> B, sorted by label):")
        for name in sorted({e["a"] for e in report} | {e["b"] for e in report}):
            print(f"  {name:24s} {paths.get(name, '(path not recorded)')}")
