...
boundary and are consecutive; M1 touches the default arm and the
vendored engine, after the technique is proven on KCP; S1 last (opt-in
arm). N1 was dropped from the sequence after its read-path audit — the
premise did not survive the e463391 fast path that landed before it.
Expected magnitude is honest: L1 ≈ 0.2-0.3
...
would show), M1 ≈ 2-5% CPU on the default arms. Not a step change;
